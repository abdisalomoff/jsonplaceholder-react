...
import ContactList from "./TodosCard";
...
const TodoForm = () => {
  ...
  const submit = (event) => {
    ...
    if (firstName && lastName && telNumber && selectedCategory) {
      ...
      if (selected === null) {
        ...
        newTodo = todos.map((contact) =>
          contact.id === selected ? { ...contact, ...todo } : contact
        );
        setSelected(null);
      }
      setTodos(newTodo);
  ...
  const editTodo = (id) => {
    const { name, lastname, telephone, category } = todos.find(
      (todo) => todo.id === id
    );
  ...
  return (
    <section className="container"  style={{ marginTop: "90px" }}>
      ...
      >
        ...
        <Button className={`btn w-100 ${selected === null ? 'btn-success' : 'btn-primary'}`} type="submit">
            {selected === null ? "Add" : "Save"} contact
        </Button>

      </Form>
      <div className="mt-5 mb-4">{mappingTodos}</div>
    </section>
  );
};
